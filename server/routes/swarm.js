...
// Deploy a stack from a pasted compose file (the file is written locally; `docker stack deploy` targets
// the active daemon — local, or remote via DOCKER_HOST=ssh from buildCliEnv). Works on remote swarms too.
router.post('/stacks/deploy', async (req, res) => {
  try {
    ...
    const { env } = buildCliEnv(dockerService.getActiveServerId(), 'stacks'); // throws 400 if remote auth unsupported
    const { stdout, stderr } = await execFileAsync('docker', ['stack', 'deploy', '-c', 'docker-compose.yml', '--detach=true', name], { cwd: dir, env, maxBuffer: 2 * 1024 * 1024 });
...
// Remove a stack via the host `docker stack rm` CLI (active daemon — local or remote via DOCKER_HOST=ssh).
router.delete('/stacks/:name', async (req, res) => {
  try {
    ...
    const { env } = buildCliEnv(dockerService.getActiveServerId(), 'stacks');
    const { stdout, stderr } = await execFileAsync('docker', ['stack', 'rm', req.params.name], { env, maxBuffer: 2 * 1024 * 1024 });
